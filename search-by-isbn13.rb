...
unless errors.empty?
  ...
  (body % "Item").tap { |item|
    # FMT=Title @@@@ Author @@@@ Creater(Role) @@@@ PublicationDate @@@@ Publisher @@@@ NumberOfPages @@@@ EAN @@@@ ASIN @@@@ Binding @@@@ Amount @@@@ DetailedPageURL
    _creator = (item / "Creator")
    creator = if _creator.text.empty?
                _creator
              else
                _creator.map{|c| "#{c.text}(#{c.attribute("Role").text})" }.join(", ")
              end
    puts "\
#{(item % "Title").text} @@@@ #{(item / "Author").map(&:text).join(", ")} @@@@ #{creator} @@@@ \
#{(item / "PublicationDate").text } @@@@ #{(item / "Publisher").text} @@@@ \
#{(item / "NumberOfPages").text } @@@@ #{(item / "EAN").text} @@@@ \
#{(item / "ASIN").text} @@@@ #{(item / "Binding").text} @@@@ \
#{(item / "Amount").text} @@@@ #{(item / "DetailPageURL").text}\
"
}
end
